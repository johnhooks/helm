...
export function ShipActionErrorFallback( { error }: { error: unknown } ) {
	const safeError = HelmError.safeFrom( error, ErrorCode.ActionsRenderFailed, __( 'Ship log entry unreadable — renderer fault', 'helm' ) );
	...
	return (
		<LogCard
			...
			tone="danger"
			status={ <StatusBadge tone="danger" size="sm">{ __( 'Fault', 'helm' ) }</StatusBadge> }
		>
			{ detail && <p>{ detail }</p> }

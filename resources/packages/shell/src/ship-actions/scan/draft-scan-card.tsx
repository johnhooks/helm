...
export function DraftScanCard( { draft, targetName, onCancel, onSubmit, isSubmitting }: DraftScanCardProps ) {
	const tone = 'lilac';
		...
		<LogCard
			...
			action={
				<div className={ `helm-tone--${ tone } helm-flex helm-items-center helm-justify-end helm-gap-x-2` }>
					...
					<Button size="sm" variant="primary" onClick={ onSubmit } disabled={ isSubmitting }>{ __( 'Begin Scan', 'helm' ) }</Button>
				</div>
			}
		>

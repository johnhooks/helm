...
import { Button, LogCard, Readout, StatusBadge, SystemCell, SystemGrid } from '@helm/ui';
...
export function DraftJumpCard( { draft, targetName, onCancel, onSubmit, isSubmitting }: DraftJumpCardProps ) {
	const tone = 'sky';
		...
		<LogCard
			...
			action={
				<div className={ `helm-tone--${ tone } helm-flex helm-items-center helm-justify-end helm-gap-x-2` }>
					<Button size="sm" variant="tertiary" onClick={ onCancel } disabled={ isSubmitting }>{ __( 'Cancel', 'helm' ) }</Button>
					<Button size="sm" variant="primary" onClick={ onSubmit } disabled={ isSubmitting }>{ __( 'Confirm Jump', 'helm' ) }</Button>
				</div>
			}
		>

...
import { Button, Countdown, LogCard, Readout, SystemCell, SystemGrid } from '@helm/ui';
...
export function ActiveJumpCard( { action, targetName }: { action: ShipAction< 'jump' >; targetName: string } ) {
	const tone = 'sky';
				...
				<SystemGrid columns={ 3 } gap="sm">
					...
					<SystemCell>
						<Readout
							...
							unit={ result?.core_cost !== undefined ? '%' : undefined }
							tone="gold"
							size="sm"
						/>
					</SystemCell>
...
function ActiveJumpCountdown( { action, tone }: { action: ShipAction< 'jump' > & { status: 'pending' | 'running' }; tone: LcarsTone } ) {

...
describe('PropulsionSystem', () => {
	it('getPerformanceRatio delegates to perfRatio formula', () => {
		// coreOutput=1.0, normal mode output mult=1.0 → output=1.0
	...
	it('getJumpDuration computes travel time', () => {
		...
		const { sys } = createPropulsionSystem();
		expect(sys.getJumpDuration(5)).toBe(22500);
	});

	it('getJumpCoreCost scales with distance', () => {
		const { sys } = createPropulsionSystem();
		const cost5 = sys.getJumpCoreCost(5);
		const cost10 = sys.getJumpCoreCost(10);

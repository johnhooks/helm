...
function circlePointsXY(radius: number, segments: number): Vector3[] {
	...
	for (let i = 0; i <= segments; i++) {
		const theta = (i / segments) * Math.PI * 2;
		points.push(
			new Vector3(Math.cos(theta) * radius, Math.sin(theta) * radius, 0)
		);
	}
	return points;
}
...
export function DistanceRings({
	...
	return (
		<group>
			{ringData.map((ring) => {
				const horizontalColor = ring.isMajor
					? MAJOR_HORIZONTAL_COLOR
					: MINOR_HORIZONTAL_COLOR;
				const lineWidth = ring.isMajor
					? MAJOR_LINE_WIDTH
					: MINOR_LINE_WIDTH;
				const labelColor = ring.isMajor
					? MAJOR_LABEL_COLOR
					: MINOR_LABEL_COLOR;

...
const denseReachableNodeIds = new Set([
	...
	...denseStars
		.filter((s) => Math.sqrt(s.x ** 2 + s.y ** 2 + s.z ** 2) <= JUMP_RANGE)
		.map((s) => s.node_id),
]);
...
export const Interactive: Story = {
	...
	render: () => {
		const [selectedStarId, setSelectedStarId] = useState<number | null>(
			null
		);
		...
		return (
			...
			>
				...
				>
					...
					<div style={{ color: '#a39a88' }}>
						{hoverInfo || 'Hover over a star'}
					</div>
				</div>
			</div>
		);
	...
	[
		{
			mode: 'perspective',
			label: 'Perspective',
			description: 'Standard 3D view with depth',
		},
		{
			mode: 'orthographic',
			label: 'Orthographic',
			description: 'Flat projection, no distortion',
		},
		{
			mode: 'narrow',
			label: 'Narrow FOV',
			description: 'Reduced perspective warping',
		},
	];

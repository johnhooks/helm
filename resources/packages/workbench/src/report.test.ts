...
describe('computeShipReport', () => {
	...
	describe('hull mass affects jump comfort range', () => {
		...
		it('heavy hull increases strain at same distance', () => {
			...
			const pioneerStrain7 = pioneerReport.jump.sampleJumps.find((j) => j.distance === 7)?.strain ?? 0;
			const bulwarkStrain7 = bulwarkReport.jump.sampleJumps.find((j) => j.distance === 7)?.strain ?? 0;
	...
	describe('hull amplitude multiplier', () => {
		it('Scout amplitude bonus reduces jump duration vs Pioneer', () => {
			...
			const pioneerDuration = pioneerReport.jump.sampleJumps.find((j) => j.distance === 1)!.duration;
			const scoutDuration = scoutReport.jump.sampleJumps.find((j) => j.distance === 1)!.duration;
	...
	describe('mechanics', () => {
		...
		describe('Aegis Testudo — Transit Shield Harmonics', () => {
			...
			it('rate is half of shield regen', () => {
				expect(report.mechanics.transitShieldRegen!.regenRateInTransit).toBe(report.shield.regenRate * 0.5);
			});

			it('sample jumps match jump section distances', () => {
				const mechDistances = report.mechanics.transitShieldRegen!.sampleJumps.map((j) => j.distance);
				const jumpDistances = report.jump.sampleJumps.map((j) => j.distance);
				expect(mechDistances).toEqual(jumpDistances);
			});

			it('recovered HP scales with jump duration', () => {
				const samples = report.mechanics.transitShieldRegen!.sampleJumps;
				...
				if (samples.length >= 2) {
					...
					expect(longer.shieldRecovered).toBeGreaterThan(shorter.shieldRecovered);
				}
			});
		...
		describe('Epoch Sensor 2 — Core Resonance Scanning', () => {
			...
			it('at magnitude 1.0, all cost becomes core damage', () => {
				for (const s of report.mechanics.coreResonanceScanning!.sampleScans) {
			...
			it('sample distances match scan section', () => {
				const mechDistances = report.mechanics.coreResonanceScanning!.sampleScans.map((s) => s.distance);
				const scanDistances = report.scan.sampleScans.map((s) => s.distance);
		...
		describe('DSC Shield Mk II — Sensor-Shield Coupling', () => {
			...
			it('multiplier is 1.4', () => {
				expect(report.mechanics.sensorShieldCoupling!.passiveAffinityMultiplier).toBeCloseTo(1.4);
			});

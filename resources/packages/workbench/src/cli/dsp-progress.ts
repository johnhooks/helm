...
function submarineWarfare(): Section {
	...
	checks.push({
		...
		detail: `Idle emission = ${emissionPower(
			'idle'
		)}. DSC 48hr confidence = ${pct(idleConf)}.`,
		values: {
			idleEmission: emissionPower('idle'),
			confidence48hr: r(idleConf),
		},
	});
	...
	checks.push({
		...
		detail: `Belt miner (masking=3): 6hr=${pct(minerBelt6hr)}, 24hr=${pct(
			minerBelt24hr
		)}.`,
	...
	checks.push({
		...
		detail: `PNP emission=${emissionPower('pnp_scan')}, VRS: 1-sample=${pct(
			pnpInstant
		)}, 30min=${pct(pnp30min)}. Cap=${pct(
	...
	checks.push({
		...
		detail: `Shield regen emission=${emissionPower(
			'shield_regen'
		)}, DSC 12hr = ${pct(shieldOpen12hr)}.`,
	...
	return {
		...
		description:
			'The core promise: idle ships invisible, belt miners safe, loud actions obvious.',
		checks,
	};
}
...
function sensorDifferentiation(): Section {
	...
	for (const s of sensors) {
		...
		passiveMiner2hr[s] = passiveDetection(
			miningPower,
			noise,
			aff.passive,
			hours(2),
	...
	checks.push({
		...
		verdict:
			passiveMiner2hr.dsc > passiveMiner2hr.vrs &&
			passiveMiner2hr.vrs > passiveMiner2hr.acu
				? 'PASS'
				: 'FAIL',
		detail: `2hr mining passive: DSC=${pct(passiveMiner2hr.dsc)}, VRS=${pct(
			passiveMiner2hr.vrs
		)}, ACU=${pct(passiveMiner2hr.acu)}.`,
		values: passiveMiner2hr,
	});

	// ACU should have meaningfully lower passive detection
	const acuPassive6hr = passiveDetection(
		miningPower,
		noise,
		SENSOR_AFFINITIES.acu.passive,
		hours(6),
		samplePeriod,
		matchedFilterGain(SENSOR_AFFINITIES.acu, 'continuous'),
		0
	);
	checks.push({
		goal: 'ACU is genuinely poor at passive detection (< 20% for miner at 6hr)',
		verdict: assess(acuPassive6hr < 0.2, acuPassive6hr < 0.4),
		detail: `ACU 6hr passive miner = ${pct(
			acuPassive6hr
		)}. ACU should need active scanning.`,
	...
	return {
		...
		description:
			'ACU = loud hunter, DSC = patient listener, VRS = generalist. Each should create a distinct playstyle.',
		checks,
	};
}
...
function driveEnvelopes(): Section {
	...
	const milConfPeak1 = passiveDetection(
		milAtPeak,
		noise,
		dsc.passive,
		samplePeriod,
	...
	const milConfPeak3 = passiveDetection(
		milAtPeak,
		noise,
		dsc.passive,
		samplePeriod * 3,
	...
	checks.push({
		...
		detail: `DR-705 peak: power=${r(milAtPeak)}, DSC 1-sample=${pct(
			milConfPeak1
		)}, 3-samples (${(samplePeriod * 3) / 60}min)=${pct(milConfPeak3)}.`,
		values: {
			power: r(milAtPeak),
			conf1: r(milConfPeak1),
			conf3: r(milConfPeak3),
		},
	});
	...
	const civAt50pct =
		envelopeAt(civEnv.spool.duration * 0.5, civEnv).power * baseEmission;
	const civConf50pct = passiveDetection(
		civAt50pct,
		noise,
		dsc.passive,
		samplePeriod,
	...
	checks.push({
		...
		detail: `DR-305 at 50% spool (${r(
			civEnv.spool.duration * 0.5
		)}s): power=${r(civAt50pct)}, confidence=${pct(civConf50pct)}.`,
	...
	checks.push({
		...
		verdict:
			milEnv.sustain.peakPower < civEnv.sustain.peakPower
				? 'PASS'
				: 'FAIL',
		detail: `DR-705 sustain=${milEnv.sustain.peakPower}, DR-305 sustain=${civEnv.sustain.peakPower}.`,
		values: {
			military: milEnv.sustain.peakPower,
			civilian: civEnv.sustain.peakPower,
		},
	});
		...
		checks.push({
			...
			verdict:
				...
				totalCooldown <= 300
					? 'PASS'
					: 'WARN',
			detail: `Spool=${totalSpool}s (${r(
				totalSpool / 60,
				1
			)}min), cooldown=${totalCooldown}s (${r(
				totalCooldown / 60,
				1
			)}min). Should be 1-5 min for catchable detection windows.`,
	...
	return {
		...
		description:
			'Each drive class should have a distinctive EM signature. Military = brutal/fast, civilian = gentle/slow, industrial = hot/heavy.',
		checks,
	};
}
...
function signatureAnalysis(): Section {
	...
	for (const tc of testCases) {
		...
		if (confidence <= 0 || confidence >= 1) {
			checks.push({
				...
				verdict: 'WARN',
				detail: `Confidence is ${
					confidence >= 1 ? '100%' : '0%'
				} — sigmoid saturated, estimation degrades at extremes. ${
					confidence >= 1
						? 'Signal so loud that inversion returns Infinity.'
						: 'Signal too weak to estimate.'
				}`,
				values: {
					truePower: tc.power,
					confidence: r(confidence),
					estimated: confidence >= 1 ? Infinity : 0,
				},
			});
			continue;
		}
		...
		checks.push({
			...
			detail: atCap
				? `At passive cap (${pct(confidence)}). Estimated=${r(
						estimated
				  )} (underestimates true power=${
						tc.power
				  }). Use active scan for precise readings.`
				: `Confidence=${pct(confidence)}, estimated=${r(
						estimated
				  )}, error=${r(error)}, top_match=${classification[0].type}.`,
			values: {
				truePower: tc.power,
				confidence: r(confidence),
				estimated: r(estimated),
				error: r(error),
				topMatch: classification[0].type,
				atCap,
			},
		});
	}
	...
	for (const key of driveKeys) {
		driveProfiles[key] = {
			base:
				envelopePeakPower(DEFAULT_DRIVE_ENVELOPES[key]) * baseEmission,
			spectralType: 'pulse',
		};
	}
	...
	for (const key of driveKeys) {
		...
		driveResults.push(
			`${env.label}: peak=${r(peakPower)}, conf=${pct(
				confidence
			)}, identified=${
				correct ? 'YES' : 'NO (guessed ' + guessedKey + ')'
			}`
		);
	}
	...
	return {
		...
		description:
			'Passive observation should reveal information: emission power estimation, activity classification, drive identification.',
		checks,
	};
}
...
function informationHierarchy(): Section {
	...
	const conf30m = passiveDetection(
		miningPower,
		noise,
		dsc.passive,
		hours(0.5),
	...
	const conf2hr = passiveDetection(
		miningPower,
		noise,
		dsc.passive,
		hours(2),
	...
	const conf6hr = passiveDetection(
		miningPower,
		noise,
		dsc.passive,
		hours(6),
	...
	const conf12hr = passiveDetection(
		miningPower,
		noise,
		dsc.passive,
		hours(12),
	...
	checks.push({
		goal: `Progressive revelation: 30m → anomaly, 2hr → type, 4hr → cap (${pct(
			cap
		)})`,
		verdict: assess(
			conf30m < 0.3 && conf2hr > 0.5 && conf6hr >= cap,
			conf30m < 0.5 && conf6hr > 0.5
		),
		detail: `Open miner vs DSC: 30m=${pct(conf30m)}, 2hr=${pct(
			conf2hr
		)}, 6hr=${pct(conf6hr)}, 12hr=${pct(conf12hr)}. Passive cap=${pct(
			cap
		)}.`,
		values: {
			conf30m: r(conf30m),
	...
	if (conf2hr > 0 && conf2hr < 1) {
		const est2hr = estimateEmissionPower(
			conf2hr,
			noise,
			dsc.passive,
			hours(2),
		...
		const runnerDistance2hr =
			class2hr.length > 1 ? class2hr[1].distance : Infinity;
		const separation2hr = runnerDistance2hr - topDistance2hr;

		const est6hr = estimateEmissionPower(
			conf6hr,
			noise,
			dsc.passive,
			hours(6),
		...
		const runnerDistance6hr =
			class6hr.length > 1 ? class6hr[1].distance : Infinity;
		...
		checks.push({
			...
			detail: `2hr: top=${class2hr[0].type} (sep=${r(
				separation2hr
			)}). 6hr: top=${class6hr[0].type} (sep=${r(
				separation6hr
			)}). Wider separation = more confident classification.`,
			values: {
				sep2hr: r(separation2hr),
				sep6hr: r(separation6hr),
				top2hr: class2hr[0].type,
				top6hr: class6hr[0].type,
			},
		});
	}
	...
	checks.push({
		...
		verdict:
			pnpEmission > surveyEmission && surveyEmission > 0
				? 'PASS'
				: 'FAIL',
	...
	return {
		...
		description:
			'From docs/dev/dsp.md: "Integration refines classification over time." Patient observation → awareness → identification.',
		checks,
	};
}
...
function informationTiers(): Section {
	...
	checks.push({
		...
		verdict: assess(
			pnpTier1 !== null && pnpTier30m === 'analysis',
			pnpTier30m === 'type' || pnpTier30m === 'analysis'
		),
		detail: `PNP VRS: 1-sample=${pct(pnpConf1)} [${
			pnpTier1 ?? 'none'
		}], 30min=${pct(pnpConf30m)} [${pnpTier30m ?? 'none'}].`,
		values: {
			conf1: r(pnpConf1),
			tier1: pnpTier1,
			conf30m: r(pnpConf30m),
			tier30m: pnpTier30m,
		},
	});
	...
	checks.push({
		...
		verdict: assess(
			tierTimeline.some((t) => t.tier === 'class') &&
				tierTimeline.some((t) => t.tier === 'analysis'),
			tierTimeline.some((t) => t.tier !== null)
		),
		detail: tierTimeline
			.map((t) => `${t.hours}hr=${pct(t.confidence)}[${t.tier ?? '-'}]`)
			.join(', '),
		values: Object.fromEntries(
			tierTimeline.map((t) => [
				`${t.hours}hr`,
				{ conf: t.confidence, tier: t.tier },
			])
		),
	});

	// Drive spool at peak: DSC should reach type or analysis tier
	const driveSpoolPeak =
		envelopePeakPower(DEFAULT_DRIVE_ENVELOPES['dr-705']) *
		DEFAULT_EMISSION_PROFILES.drive_spool.base;
	const driveConf = passiveDetection(
		driveSpoolPeak,
		noise,
		dsc.passive,
		samplePeriod,
	...
	checks.push({
		...
		verdict: assess(
			driveTier === 'analysis' || driveTier === 'type',
			driveTier === 'class'
		),
		detail: `DR-705 peak DSC 1-sample: confidence=${pct(driveConf)}, tier=${
			driveTier ?? 'none'
		}.`,
	...
	const correlatorThresholds = adjustedThresholds(
		DEFAULT_TIER_THRESHOLDS,
		0.05
	);
	...
	checks.push({
		...
		verdict:
			boostedTier !== null && defaultTier !== boostedTier
				? 'PASS'
				: 'WARN',
		detail: `Confidence=${pct(marginalConf)}: default=${
			defaultTier ?? 'none'
		}, with correlator=${
			boostedTier ?? 'none'
		}. Thresholds shifted by 0.05.`,
	...
	return {
		...
		description:
			'Confidence maps to information quality: anomaly → class → type → analysis. Equipment and experience shift thresholds.',
		checks,
	};
}
...
function implementationCoverage(): Section {
	...
	const modules = [
		...
		{
			name: 'noise.ts',
			desc: 'Noise floor (stellar + ships + belt + ECM + shipNoiseFactor)',
			status: 'done',
		},
		{
			name: 'snr.ts',
			desc: 'SNR, masked SNR, integration gain',
			status: 'done',
		},
		{
			name: 'detection.ts',
			desc: 'Detection sigmoid, cumulative detection, matched filter',
			status: 'done',
		},
		{
			name: 'passive.ts',
			desc: 'Full passive pipeline, multi-source report',
			status: 'done',
		},
		{
			name: 'envelope.ts',
			desc: 'Drive ADSR envelope (spool/sustain/cooldown)',
			status: 'done',
		},
		{
			name: 'signature.ts',
			desc: 'Inverse sigmoid, power estimation, classification',
			status: 'done',
		},
		{
			name: 'classification.ts',
			desc: 'Information tiers, adjusted thresholds (equipment/experience)',
			status: 'done',
		},
	];
	...
	const notYet = [
		{
			feature: 'Active scan sweep model',
			doc: 'envelopes.md',
			desc: 'Sensor LFO sweep pattern, checkpoint-based detection with cumulative probability per sweep',
		},
		{
			feature: 'Self-interference',
			doc: 'dsp.md',
			desc: 'Own-ship emissions degrade own sensor SNR (drive cooldown degrades scanning)',
		},
		{
			feature: 'ECM/ECCM',
			doc: 'scanning.md',
			desc: 'Intentional noise injection and counter-measures',
		},
		{
			feature: 'Signal processor addon',
			doc: 'scanning.md',
			desc: 'Correlator/amplifier/filter equipment modifying the signal chain',
		},
		{
			feature: 'Filter tuning',
			doc: 'dsp.md',
			desc: 'Bandwidth, noise rejection, spectral focus as tunable parameters',
		},
		{
			feature: 'Component drift',
			doc: 'envelopes.md',
			desc: 'Usage-based template sharpening (veteran sensors detect familiar signals better)',
		},
		{
			feature: 'Chirp pattern fingerprinting',
			doc: 'dsp.md',
			desc: 'Drive frequency sweep rate as a unique identifier per drive model',
		},
		{
			feature: 'Distance/range within system',
			doc: 'scanning.md',
			desc: 'Optional: spatial attenuation within a system abstraction',
		},
		{
			feature: 'Information decay',
			doc: 'scanning.md',
			desc: 'Stale contacts lose confidence over time',
		},
	];
...
function tuningHealth(): Section {
	...
	checks.push({
		...
		verdict:
			...
			top3[2] === 'drive_spool'
				? 'PASS'
				: 'WARN',
		detail: `Top 3: ${emissions
			.slice(0, 3)
			.map((e) => `${e.type}=${e.power}`)
			.join(', ')}. Bottom: ${emissions
			.slice(-3)
			.map((e) => `${e.type}=${e.power}`)
			.join(', ')}.`,
	});
	...
	checks.push({
		...
		verdict: 'INFO',
		detail: `Raw SNR=${r(rawSNR)}. Integrated: ${integrationPoints
			.map((p) => `${p.hours}hr=${p.intSNR}`)
			.join(', ')}.`,
		values: Object.fromEntries(
			integrationPoints.map((p) => [`${p.hours}hr`, p.intSNR])
		),
	});
	...
	checks.push({
		...
		verdict: assess(
			transitionWidth >= 4.0 && transitionWidth <= 12.0,
			transitionWidth >= 2.0
		),
		detail: `10%=${r(snr10)}, 50%=${r(snr50)}, 90%=${r(snr90)}, width=${r(
			transitionWidth
		)}. Steepness=${
			DEFAULT_DSP_CONSTANTS.detectionSteepness
		}. Target: 4-12 width (very gradual).`,
		values: {
			snr10: r(snr10),
			snr50: r(snr50),
			snr90: r(snr90),
			width: r(transitionWidth),
		},
	});
	...
	return {
		...
		description:
			'Validates that the constants produce reasonable gameplay characteristics.',
		checks,
	};
}
...
function envelopeTimingProblem(): Section {
	...
	checks.push({
		...
		verdict: 'INFO',
		detail: `Sample period=${samplePeriod}s (${
			samplePeriod / 60
		}min). Spool durations: ${DRIVE_KEYS.map(
			(k) =>
				`${DEFAULT_DRIVE_ENVELOPES[k].label}=${
					DEFAULT_DRIVE_ENVELOPES[k].spool.duration
				}s (${r(DEFAULT_DRIVE_ENVELOPES[k].spool.duration / 60, 1)}min)`
		).join(', ')}. Spool/sample ratio: ${r(
			(DEFAULT_DRIVE_ENVELOPES['dr-705'].spool.duration / samplePeriod) *
				100,
			0
		)}%-${r(
			(DEFAULT_DRIVE_ENVELOPES['dr-505'].spool.duration / samplePeriod) *
				100,
			0
		)}%.`,
		values: {
			samplePeriodS: samplePeriod,
			spoolDurations: Object.fromEntries(
				DRIVE_KEYS.map((k) => [
					k,
					DEFAULT_DRIVE_ENVELOPES[k].spool.duration,
				])
	...
	for (const key of DRIVE_KEYS) {
		...
		const confAtPeak = passiveDetection(
			peakPower,
			noise,
			dsc.passive,
			samplePeriod,
		...
		const confAtAvg = passiveDetection(
			avgPowerInSample,
			noise,
			dsc.passive,
			samplePeriod,
		...
		checks.push({
			...
			detail: `Envelope avg power=${r(
				avgEnvelopePower
			)}, duty cycle=${pct(dutyCycle)}, diluted power=${r(
				avgPowerInSample
			)}. Peak conf=${pct(confAtPeak)}, time-avg conf=${pct(confAtAvg)}.`,
			values: {
	...
	checks.push({
		...
		verdict: 'INFO',
		detail: periodResults
			.map((p) => `${p.period}s=${pct(p.conf)}`)
			.join(', '),
		values: Object.fromEntries(
			periodResults.map((p) => [`${p.period}s`, p.conf])
		),
	});
	...
	checks.push({
		...
		verdict: 'INFO',
		detail: `Spools are now ${
			DEFAULT_DRIVE_ENVELOPES['dr-705'].spool.duration / 60
		}-${
			DEFAULT_DRIVE_ENVELOPES['dr-505'].spool.duration / 60
		} minutes. With 5-min default sampling, a military spool occupies ${r(
			(DEFAULT_DRIVE_ENVELOPES['dr-705'].spool.duration / samplePeriod) *
				100,
			0
		)}% of a sample window. A 1-min scanner catches every spool. A 10-min scanner has a ${r(
			(DEFAULT_DRIVE_ENVELOPES['dr-705'].spool.duration / 600) * 100,
			0
		)}% chance. Scan frequency is now a meaningful tactical choice.`,
	});

	return {
		title: 'Envelope vs Sample Period — Scan Frequency Tradeoffs',
		description:
			'Spool durations are minutes. Passive samples every 1-10 minutes. Faster scanning catches more transients.',
		checks,
	};
}
...
function selfInterference(): Section {
	...
	for (const key of DRIVE_KEYS) {
		...
		for (const frac of points) {
			...
			const dirtyConf = passiveDetection(
				miningPower,
				dirtyNoise,
				dsc.passive,
				hours(1),
			...
			const degradation =
				cleanConf > 0 ? (cleanConf - dirtyConf) / cleanConf : 0;
		...
		checks.push({
			...
			verdict: 'INFO',
			detail: `Self-emission during cooldown degrades own sensor. ${
				env.label
			} cooldown peak=${env.cooldown.peakPower * baseEmission}.`,
			values: results,
		});
	}
	...
	checks.push({
		...
		detail: `DSC detecting miner 2hr: clean=${pct(
			cleanConf
		)}, during DR-705 cooldown peak=${pct(
			hotConf
		)}. Self-emission=${milCooldownPeak}.`,
		values: {
			clean: r(cleanConf),
			hot: r(hotConf),
			selfEmission: milCooldownPeak,
		},
	});
	...
	const civHotConf = passiveDetection(
		miningPower,
		civHotNoise,
		dsc.passive,
		hours(2),
	...
	checks.push({
		...
		detail: `DSC detecting miner 2hr: clean=${pct(
			cleanConf
		)}, DR-305 cooldown=${pct(civHotConf)}, DR-705 cooldown=${pct(
			hotConf
		)}. Civilian degrades less than military.`,
		values: {
			clean: r(cleanConf),
			civHot: r(civHotConf),
			milHot: r(hotConf),
	...
	return {
		...
		description:
			'From dsp.md: own-ship emissions add to the noise the sensor must filter. Military drives arrive hot.',
		checks,
	};
}
...
function gameplayScenarios(): Section {
	...
	// Question: does the miner get warning before the wolf's scan finishes?
	{
		...
		const minerDetectsPNP = passiveDetection(
			pnpPower,
			noise,
			vrs.passive,
			samplePeriod,
		...
		const minerDetectsPNP3 = passiveDetection(
			pnpPower,
			noise,
			vrs.passive,
			samplePeriod * 3,
		...
		const minerDetectsPNP6 = passiveDetection(
			pnpPower,
			noise,
			vrs.passive,
			samplePeriod * 6,
		...
		checks.push({
			...
			detail: `Wolf PNP (power=${pnpPower}). VRS: 1 sample=${pct(
				minerDetectsPNP
			)}, 3 samples (${(samplePeriod * 3) / 60}min)=${pct(
				minerDetectsPNP3
			)}, 6 samples (${(samplePeriod * 6) / 60}min)=${pct(
				minerDetectsPNP6
			)}. Awareness builds, not instant.`,
			values: {
				pnpPower,
				conf1: r(minerDetectsPNP),
				conf3: r(minerDetectsPNP3),
				conf6: r(minerDetectsPNP6),
				sampleMinutes: samplePeriod / 60,
			},
		});
	}
	...
	// How much do they raise the noise floor for each other?
	{
		...
		const noise3wolves = noiseFloor(baseline, [
			pnpPower,
			pnpPower,
			pnpPower,
		]);
		...
		const minerVs1wolf = passiveDetection(
			minerSignal,
			noise1wolf,
			dsc.passive,
			hours(6),
		...
		const minerVs3wolves = passiveDetection(
			minerSignal,
			noise3wolves,
			dsc.passive,
			hours(6),
		...
		const minerVsClean = passiveDetection(
			minerSignal,
			noiseClean,
			dsc.passive,
			hours(6),
		...
		checks.push({
			...
			detail: `DSC finding miner (6hr): clean=${pct(
				minerVsClean
			)}, 1 wolf PNP=${pct(minerVs1wolf)}, 3 wolf PNP=${pct(
				minerVs3wolves
			)}. Wolf pack noise helps hide the miner.`,
			values: {
				clean: r(minerVsClean),
				wolf1: r(minerVs1wolf),
				wolf3: r(minerVs3wolves),
			},
		});
		...
		const minerDetectsPackInstant = passiveDetection(
			totalWolfSignal,
			noiseClean,
			1.0,
			samplePeriod,
		...
		const minerDetectsPack15min = passiveDetection(
			totalWolfSignal,
			noiseClean,
			1.0,
			samplePeriod * 3,
		...
		checks.push({
			...
			detail: `3-wolf PNP signal (RMS)=${r(
				totalWolfSignal
			)} vs solo=${pnpPower}. 1 sample=${pct(
				minerDetectsPackInstant
			)}, 15min=${pct(
				minerDetectsPack15min
			)}. Pack can't sneak for long.`,
			values: {
				soloPower: pnpPower,
				packPower: r(totalWolfSignal),
				conf1: r(minerDetectsPackInstant),
				conf15m: r(minerDetectsPack15min),
			},
		});
	}
	...
	// The spool duration IS the warning window.
	{
		...
		for (const key of DRIVE_KEYS) {
			...
			checks.push({
				...
				detail: `Spool lasts ${warningWindow}s (${r(
					warningWindow / 60,
					1
				)}min). Sample period=${samplePeriod}s (${
					samplePeriod / 60
				}min). Catch probability: ${pct(catchProbability)}.`,
				values: {
					warningWindowS: warningWindow,
					samplePeriodS: samplePeriod,
					catchProbability: r(catchProbability),
				},
			});
		}
	}
	...
	{
		const minerEmissions = Array(10).fill(
			emissionPower('mining')
		) as number[];
		const wolfEmissions = [
			emissionPower('pnp_scan'),
		...
		const minerConf = passiveDetection(
			emissionPower('mining'),
			noise,
			dsc.passive,
			hours(6),
		...
		const wolfConf = passiveDetection(
			emissionPower('pnp_scan'),
			noise,
			1.0,
			samplePeriod,
		...
		const wolfConf30m = passiveDetection(
			emissionPower('pnp_scan'),
			noise,
			1.0,
			samplePeriod * 6,
		...
		checks.push({
			...
			detail: `Noise floor=${r(
				noise
			)} (baseline=${baseline}). Miner detection (DSC 6hr): ${pct(
				minerConf
			)}. Wolf PNP (VRS 1 sample): ${pct(wolfConf)}, 30min: ${pct(
				wolfConf30m
			)}.`,
			values: {
				noiseFloor: r(noise),
				minerConf: r(minerConf),
				wolfConf1: r(wolfConf),
				wolfConf30m: r(wolfConf30m),
			},
		});
	}
	...
	// Timestamp model means their envelopes are independent.
	{
		...
		checks.push({
			...
			verdict: 'INFO',
			detail: `DR-705 at 5s (${stateA.phase}): emission=${r(
				emissionA
			)}. DR-305 at 40s (${stateB.phase}): emission=${r(
				emissionB
			)}. Combined noise floor=${r(totalNoise)}.`,
			values: {
				shipA: {
					elapsed: 5,
					phase: stateA.phase,
					emission: r(emissionA),
				},
				shipB: {
					elapsed: 40,
					phase: stateB.phase,
					emission: r(emissionB),
				},
	...
	return {
		...
		description:
			'Real gameplay situations stress-tested against the timestamp model. Wolves, miners, wolf packs, busy systems.',
		checks,
	};
}

...
const VERDICT_RANK: Record<string, number> = { FAIL: 1, WARN: 2, PASS: 3, INFO: 0 };

function verdictRegression(before: string, after: string): { regression: boolean; improvement: boolean } {
	const bRank = VERDICT_RANK[before] ?? 0;
	const aRank = VERDICT_RANK[after] ?? 0;
	// INFO transitions are always neutral
	if (bRank === 0 || aRank === 0) { return { regression: false, improvement: false }; }
	return {
...
function diffDspProgress(before: DspOutput, after: DspOutput): DiffEntry[] {
	...
	for (const [title, beforeSection] of beforeSections) {
		...
		const afterChecks = new Map(afterSection.checks.map((c) => [c.goal, c]));
		const beforeChecks = new Map(beforeSection.checks.map((c) => [c.goal, c]));

		for (const [goal] of beforeChecks) {
			if (!afterChecks.has(goal)) {
				entries.push({ path: `${title} > ${goal}`, type: 'removed', before: goal });
			}
		}

		for (const [goal] of afterChecks) {
			if (!beforeChecks.has(goal)) {
				entries.push({ path: `${title} > ${goal}`, type: 'added', after: goal });
			}
		}
		...
		for (const [goal, beforeCheck] of beforeChecks) {
			...
			if (beforeCheck.verdict !== afterCheck.verdict) {
				const { regression, improvement } = verdictRegression(beforeCheck.verdict, afterCheck.verdict);
				entries.push({
					path, type: 'changed', field: 'verdict',
					before: beforeCheck.verdict, after: afterCheck.verdict,
			...
			if (beforeCheck.values && afterCheck.values) {
				diffValues(path, beforeCheck.values, afterCheck.values, entries);
			}
		}
	}
...
function diffValues(
	...
	for (const key of allKeys) {
		...
		if (bVal === undefined) {
			entries.push({ path: basePath, type: 'added', field: key, after: aVal });
			continue;
		}
		if (aVal === undefined) {
			entries.push({ path: basePath, type: 'removed', field: key, before: bVal });
			continue;
		}
		if (typeof bVal === 'object' && typeof aVal === 'object' && bVal !== null && aVal !== null) {
			diffValues(`${basePath} > ${key}`, bVal as Record<string, unknown>, aVal as Record<string, unknown>, entries);
			continue;
		}
		if (bVal !== aVal) {
			const delta = numericDelta(bVal, aVal);
			entries.push({ path: basePath, type: 'changed', field: key, before: bVal, after: aVal, delta });
		}
	}
}
...
function diffAnalyse(before: AnalyseOutput, after: AnalyseOutput): DiffEntry[] {
	...
	for (const [cat, beforeCat] of beforeCats) {
		...
		const afterScenarios = new Map(afterCat.scenarios.map((s) => [s.name, s]));
		const beforeScenarios = new Map(beforeCat.scenarios.map((s) => [s.name, s]));

		for (const [name] of beforeScenarios) {
			if (!afterScenarios.has(name)) {
				entries.push({ path: `${cat} > ${name}`, type: 'removed', before: name });
			}
		}
		for (const [name] of afterScenarios) {
			if (!beforeScenarios.has(name)) {
				entries.push({ path: `${cat} > ${name}`, type: 'added', after: name });
			}
		}
		...
		for (const [name, beforeScenario] of beforeScenarios) {
			...
			diffNumericLeaves(`${cat} > ${name}`, beforeScenario.output, afterScenario.output, entries);
		}
	}
...
function diffNumericLeaves(
	...
	if (typeof before === 'number' && typeof after === 'number') {
		if (before !== after) {
			...
			entries.push({ path: basePath, type: 'changed', before, after, delta });
		}
		return;
	}
	...
	if (typeof before === 'object' && typeof after === 'object' && before !== null && after !== null) {
		if (Array.isArray(before) && Array.isArray(after)) {
			...
			for (let i = 0; i < len; i++) {
				if (i >= before.length) {
					entries.push({ path: `${basePath}[${i}]`, type: 'added', after: after[i] });
				} else if (i >= after.length) {
					entries.push({ path: `${basePath}[${i}]`, type: 'removed', before: before[i] });
				} else {
					diffNumericLeaves(`${basePath}[${i}]`, before[i], after[i], entries);
				}
			}
			return;
		}
		...
		for (const key of allKeys) {
			if (!(key in bObj)) {
				entries.push({ path: `${basePath}.${key}`, type: 'added', after: aObj[key] });
			} else if (!(key in aObj)) {
				entries.push({ path: `${basePath}.${key}`, type: 'removed', before: bObj[key] });
			} else {
				diffNumericLeaves(`${basePath}.${key}`, bObj[key], aObj[key], entries);
			}
		}
		return;
	}
...
function balanceRowKey(row: BalanceRow): string {
	...
	if (row.core) { parts.push(row.core); }
	if (row.drive) { parts.push(row.drive); }
	return parts.join('+');
}
...
function diffBalance(before: BalanceOutput, after: BalanceOutput): DiffEntry[] {
	...
	const numericFields = [
		...
		'jumpComfort', 'scanComfort', 'shieldCapacity', 'shieldRegen', 'cargo', 'equipmentSlots',
	];
	...
	for (const [key, beforeRow] of beforeRows) {
		...
		for (const field of numericFields) {
			...
			if (bVal !== aVal) {
				const delta = numericDelta(bVal, aVal);
				entries.push({ path: key, type: 'changed', field, before: bVal, after: aVal, delta });
			}
		}
	}
	...
	for (const o of beforeOutliers) {
		if (!afterOutliers.has(o)) {
			entries.push({ path: 'outliers', type: 'removed', before: o, improvement: true });
		}
	}
	for (const o of afterOutliers) {
		if (!beforeOutliers.has(o)) {
			entries.push({ path: 'outliers', type: 'added', after: o, regression: true });
		}
	}
...
function diffDetection(before: DetectionOutput, after: DetectionOutput): DiffEntry[] {
	const entries: DiffEntry[] = [];

	// Diff detection matrix
	const afterMatrix = new Map(after.detectionMatrix.map((e) => [detectionEntryKey(e), e]));
	const beforeMatrix = new Map(before.detectionMatrix.map((e) => [detectionEntryKey(e), e]));

	for (const [key] of beforeMatrix) {
		if (!afterMatrix.has(key)) {
			entries.push({ path: `matrix:${key}`, type: 'removed', before: key });
		}
	}
...
function summarize(entries: DiffEntry[]): DiffResult['summary'] {
	let added = 0, removed = 0, changed = 0, regressions = 0, improvements = 0;
	for (const e of entries) {
		if (e.type === 'added') { added++; }
		else if (e.type === 'removed') { removed++; }
		else { changed++; }
		if (e.regression) { regressions++; }
		if (e.improvement) { improvements++; }
	}
	return { added, removed, changed, regressions, improvements };
}

export function diffBaseline(command: BaselineCommand, before: unknown, after: unknown): DiffResult {
	...
	switch (command) {
		case 'dsp-progress':
			...
			break;
		case 'analyse':
			entries = diffAnalyse(before as AnalyseOutput, after as AnalyseOutput);
			break;
		case 'balance':
			entries = diffBalance(before as BalanceOutput, after as BalanceOutput);
			break;
		case 'detection':
			entries = diffDetection(before as DetectionOutput, after as DetectionOutput);
			break;
	}

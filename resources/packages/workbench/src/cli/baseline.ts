...
function runCommand(command: BaselineCommand): unknown {
	switch (command) {
		case 'analyse':
			return runAnalyse();
		case 'dsp-progress':
			return runDspProgress();
		case 'balance':
			return runBalance({ flags: {}, positional: [] });
		case 'detection':
			return runDetection();
	}
}
...
function showBaseline(command: BaselineCommand): void {
	...
	if (!existsSync(path)) {
		console.error(
			`No baseline found for ${command}. Run: bun run wb baseline save ${command}`
...
function printDiffResult(result: DiffResult): void {
	...
	for (const entry of entries) {
		...
		if (entry.type === 'added') {
			...
			log(
				`${prefix}REMOVED: ${location} (was ${JSON.stringify(
					entry.before
				)})`
			);
			...
			log(
				`${prefix}${location}: ${entry.before} → ${entry.after} (${sign}${entry.delta})`
			);
		} else {
			log(
				`${prefix}${location}: ${JSON.stringify(
					entry.before
...
export function baseline(
	...
	if (!subcommand || !['save', 'show', 'diff'].includes(subcommand)) {
		console.error(
			'Usage: bun run wb baseline <save|show|diff> <command|--all>'
		); // eslint-disable-line no-console
		console.error(''); // eslint-disable-line no-console
		console.error('Commands: analyse, dsp-progress, balance, detection'); // eslint-disable-line no-console
		console.error(''); // eslint-disable-line no-console
		console.error(
			'  baseline save <command>     Save current output as baseline'
		); // eslint-disable-line no-console
		console.error('  baseline save --all         Save all baselines'); // eslint-disable-line no-console
		console.error(
			'  baseline show <command>     Print saved baseline to stdout'
		); // eslint-disable-line no-console
		console.error(
			'  baseline diff <command>     Diff current vs saved baseline'
		); // eslint-disable-line no-console
	...
	if (subcommand === 'save') {
		...
		for (const cmd of commands) {
			if (!cmd || !isBaselineCommand(cmd)) {
				console.error(
					`Unknown command: ${cmd}. Valid: ${BASELINE_COMMANDS.join(
						', '
					)}`
				); // eslint-disable-line no-console
				process.exit(1);

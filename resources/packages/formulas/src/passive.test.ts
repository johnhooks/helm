...
describe('passiveDetection', () => {
	...
	it('masking noise reduces detection', () => {
		const unmasked = passiveDetection(1.0, 1.0, 1.0, 3600, samplePeriod, 1.0, 0);
		const masked = passiveDetection(1.0, 1.0, 1.0, 3600, samplePeriod, 1.0, 3.0);
	...
	it('matched filter gain improves detection', () => {
		...
		const withGain = passiveDetection(1.0, 2.0, 1.0, 3600, samplePeriod, 1.5);
		expect(withGain).toBeGreaterThan(noGain);
	});

	it('pilot skill improves detection', () => {
		const rookie = passiveDetection(1.0, 2.0, 1.0, 3600, samplePeriod, 1.0, 0, 1.0);
		const veteran = passiveDetection(1.0, 2.0, 1.0, 3600, samplePeriod, 1.0, 0, 1.2);
	...
	it('pilot skill stacks with sensor affinity', () => {
		...
		const vrsVeteran = passiveDetection(1.0, 1.0, 1.0, 3600, samplePeriod, 1.0, 0, 1.2);
		const dscRookie = passiveDetection(1.0, 1.0, 1.2, 3600, samplePeriod, 1.0, 0, 1.0);
	...
	it('DSC detects open-space miner after hours of integration', () => {
		...
		const confidence = passiveDetection(
			1.0, 1.0, 1.4, 6 * 3600, samplePeriod, 1.5, 0,
		);
		expect(confidence).toBe(DEFAULT_DSP_CONSTANTS.passiveConfidenceCap);
	});
	...
	it('passive detection never exceeds confidence cap', () => {
		...
		const confidence = passiveDetection(100.0, 0.1, 2.0, 48 * 3600, samplePeriod, 2.0, 0, 1.5);
		expect(confidence).toBe(DEFAULT_DSP_CONSTANTS.passiveConfidenceCap);
	...
	it('belt masking dramatically slows detection', () => {
		...
		const open = passiveDetection(
			1.0, 1.0, 1.4, 3600, samplePeriod, 1.5, 0,
		);
		const belt = passiveDetection(
			1.0, 1.0, 1.4, 3600, samplePeriod, 1.5, 3.0,
		);
	...
	it('ACU is poor at passive detection', () => {
		...
		const confidence = passiveDetection(
			1.0, 1.0, 0.6, 2 * 3600, samplePeriod, 0.7, 0,
		);
		expect(confidence).toBeLessThan(0.5);
	});
});
...
describe('passiveReport', () => {
	...
	it('returns detections sorted by confidence descending', () => {
		...
		for (let i = 0; i < report.length - 1; i++) {
			expect(report[i].confidence).toBeGreaterThanOrEqual(report[i + 1].confidence);
		}
	});
	...
	it('filters out low-confidence detections', () => {
		...
		const report = passiveReport(sources, 1.0, vrs, samplePeriod, samplePeriod, 0.1);
	...
	it('applies masking noise per-source', () => {
		const sources: EmissionSource[] = [
			...
			{ power: 1.0, spectralType: 'continuous', label: 'belt_miner', maskingNoise: 3.0 },
		];
	...
	it('DSC detects more sources than ACU in passive mode', () => {
		...
		const dscReport = passiveReport(sources, 2.0, dsc, 3600, samplePeriod, 0.5);
		const acuReport = passiveReport(sources, 2.0, acu, 3600, samplePeriod, 0.5);

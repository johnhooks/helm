...
describe('reducer', () => {
	...
	describe('CREATE_DRAFT', () => {
		it('sets the draft action with isDraft flag', () => {
			const draft = {
				type: 'scan_route' as const,
				params: { target_node_id: 5 },
			};
	...
	describe('CREATE_ACTION_START', () => {
		it('sets isSubmitting and clears error', () => {
			...
			expect(state.create.error).toBeNull();
			expect(state.create.action).toEqual({
				type: 'scan_route',
				params: {},
			});
			expect(state.create.isDraft).toBe(true);
	...
	describe('CREATE_ACTION_FINISHED', () => {
		...
		it('does not affect queries for other ships', () => {
			...
			expect(state.actions.queries['/helm/v1/ships/2/actions']).toBe(
				prev.actions.queries['/helm/v1/ships/2/actions']
			);
		});
	});
	...
	describe('CREATE_ACTION_FAILED', () => {
		...
		it('does not remove existing action data', () => {
			...
			const prev = createState({
				actions: {
					byId: { 1: action },
					queries: { '/helm/v1/ships/1/actions': [1] },
				},
				create: {
					action: { type: 'scan_route' as const, params: {} },
					isDraft: true,
	...
	describe('FETCH_ACTION_START', () => {
		it('sets isLoading for the action and clears its error', () => {
			const prev = createState({
				actions: {
					error: { 42: new HelmError('helm.test', 'Previous error') },
				},
			});
	...
	describe('FETCH_ACTIONS_START', () => {
		it('sets isLoading and clears error', () => {
				...
				actions: {
					meta: {
						'/helm/v1/ships/1/actions': {
							next: 'http://test/next',
							isLoading: false,
							error: new HelmError('helm.test', 'E'),
						},
					},
				},
			});
			...
			expect(
				state.actions.meta['/helm/v1/ships/1/actions']!.isLoading
			).toBe(true);
			expect(
				state.actions.meta['/helm/v1/ships/1/actions']!.error
			).toBeNull();
			expect(state.actions.meta['/helm/v1/ships/1/actions']!.next).toBe(
				'http://test/next'
			);
		});
		...
		it('initializes meta for new query', () => {
			const state = reduce(undefined, {
				type: 'FETCH_ACTIONS_START',
				queryId: '/helm/v1/ships/5/actions',
			});
	...
	describe('FETCH_ACTIONS_FINISHED', () => {
		...
		it('sets next to null on last page', () => {
			...
			expect(
				state.actions.meta['/helm/v1/ships/1/actions']!.next
			).toBeNull();
		});
	});
	...
	describe('FETCH_ACTIONS_FAILED', () => {
		it('sets error and clears isLoading', () => {
			...
			expect(state.actions.meta['/helm/v1/ships/1/actions']!.error).toBe(
				error
			);
			expect(state.actions.meta['/helm/v1/ships/1/actions']!.next).toBe(
				'http://test/next'
	...
	describe('LOAD_MORE_FINISHED', () => {
		it('appends action IDs to existing query', () => {
			...
			const page2 = [
				createShipAction({ id: 9 }),
				createShipAction({ id: 8 }),
			];
			const state = reduce(prev, {

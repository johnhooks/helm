...
describe('createAction', () => {
	...
	it('calls apiFetch with the correct path and data', async () => {
		...
		await createAction(7, 'scan_route', { depth: 3 })({
			dispatch,
		} as never);
...
describe('submitDraft', () => {
	...
	it('calls createAction with draft type and params', async () => {
		select.getDraft.mockReturnValue({
			type: 'scan_route',
			params: { depth: 3 },
		});
		...
		expect(dispatch.createAction).toHaveBeenCalledWith(7, 'scan_route', {
			depth: 3,
		});
	});
	...
	it('throws when no draft', async () => {
		...
		await expect(
			submitDraft(7)({ dispatch, select } as never)

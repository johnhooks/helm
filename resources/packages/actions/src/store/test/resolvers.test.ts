...
describe( 'getActions resolver', () => {
	...
	it( 'dispatches START then FINISHED on success', async () => {
		...
		expect( dispatch ).toHaveBeenNthCalledWith( 1, { type: 'FETCH_ACTIONS_START', queryId: '/helm/v1/ships/1/actions' } );
	...
	it( 'dispatches FAILED on error', async () => {
		...
		expect( dispatch.mock.calls[ 1 ][ 0 ].queryId ).toBe( '/helm/v1/ships/1/actions' );
	} );
} );
...
describe( 'getAction resolver', () => {
	...
	it( 'dispatches START then FINISHED on success', async () => {
		const updated = createShipAction( { id: 42, ship_post_id: 5, status: 'fulfilled' } );
		mockedApiFetch.mockResolvedValue( updated );

		await getAction( 42 )( { dispatch } as never );

		expect( dispatch ).toHaveBeenCalledTimes( 2 );
		expect( dispatch ).toHaveBeenNthCalledWith( 1, { type: 'FETCH_ACTION_START', actionId: 42 } );
		expect( dispatch ).toHaveBeenNthCalledWith( 2, { type: 'FETCH_ACTION_FINISHED', action: updated } );
	} );

	it( 'calls apiFetch with the correct path', async () => {
		mockedApiFetch.mockResolvedValue( createShipAction( { id: 10, ship_post_id: 3 } ) );

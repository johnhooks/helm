...
export const getActions =
	...
	async ( { dispatch } ) => {
		...
		try {
			...
			dispatch( { type: 'FETCH_ACTIONS_FINISHED', queryId, actions, next: links.next ?? null } );
		} catch ( error ) {
			dispatch( {
	...
	async ( { dispatch } ) => {
		...
		try {
			...
			dispatch( {
				...
				actionId,
				error: HelmError.safe( ErrorCode.ActionsInvalidResponse, __( 'Ship link failed to retrieve action data', 'helm' ), await HelmError.asyncFrom( error ) ),
			} );
		}
	};

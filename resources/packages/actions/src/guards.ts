...
export type ActiveAction<T extends ShipActionType = ShipActionType> =
	ShipAction<T> & {
		status: 'pending' | 'running';
		result: ActionContract<T>['activeResult'] | null;
	};

export type FulfilledAction<T extends ShipActionType = ShipActionType> =
	ShipAction<T> & {
		status: 'fulfilled' | 'partial';
		result: ActionContract<T>['fulfilledResult'];
	};

export type FailedAction<T extends ShipActionType = ShipActionType> =
	ShipAction<T> & {
		status: 'failed';
...
export function isScanRoute(
	action: ShipAction
): action is ShipAction<'scan_route'>;
export function isScanRoute(
	draft: DraftAction
): draft is DraftAction<'scan_route'>;

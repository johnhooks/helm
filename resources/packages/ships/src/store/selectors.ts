...
export const getSystemsError = (state: State): HelmError | null =>
	state.systems.error;

export const getEdits = (state: State): Partial<ShipState> | null =>
	state.edits.ship;

export const isSubmitting = (state: State): boolean => state.edits.isSubmitting;

export const getEditError = (state: State): HelmError | null =>
	state.edits.error;
...
export const getShipWithLoadout = createRegistrySelector(
	(select) =>
		(state: State, _shipId: number): ShipLoadout => {
 ...
 * @throws {HelmError} When ship, systems, a required slot, or its preloaded product is missing.
 */
export const getSystemStats = createRegistrySelector((select) =>
	createSelector(

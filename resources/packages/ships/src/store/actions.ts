...
export function receiveShip(ship: WithRestLinks<ShipState>): Action {
	return { type: 'RECEIVE_SHIP', ship };
}

export function receiveSystems(systems: SystemComponentResponse[]): Action {
...
export const patchShip =
	(
		shipId: number,
		edits: Partial<ShipState>
	): ((args: StoreThunkArgs) => Promise<HelmError | null>) =>
	async ({ dispatch }) => {
		...
		try {
			...
			const safeError = HelmError.safe(
				...
				__('Ship link failed to update ship configuration', 'helm'),

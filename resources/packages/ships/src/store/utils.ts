...
import type { ShipLoadout, SystemComponentResponse, SystemSlots } from '@helm/types';
 ...
 */
export function getSystemSlots( systems: SystemComponentResponse[] ): SystemSlots {
	...
	assert( core, ErrorCode.ShipsMissingSystem, 'Ship missing required system: core' );
	assert( drive, ErrorCode.ShipsMissingSystem, 'Ship missing required system: drive' );
	assert( sensor, ErrorCode.ShipsMissingSystem, 'Ship missing required system: sensor' );
	assert( shield, ErrorCode.ShipsMissingSystem, 'Ship missing required system: shield' );
	assert( nav, ErrorCode.ShipsMissingSystem, 'Ship missing required system: nav' );
...
export function expectLoadout(
	...
	try {
		...
		assert( systems, ErrorCode.ShipsSystemsNotLoaded, 'Expected systems to be loaded' );
		...
		throw HelmError.safe(
			ErrorCode.ShipLinkLoadoutFailed,

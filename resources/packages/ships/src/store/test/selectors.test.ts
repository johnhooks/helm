			...
			if (storeDescriptor === productsStore) {
				return {
					getPreloadedProduct: (id: number) => {
						...
						if (!product) {
							throw new HelmError(
								'helm.products.not_preloaded',
								`Expected product to be preloaded: ${id}`
							);
						}
						return product;
					},
		...
		systems: {
			systems: [
				...
				createSystemComponent({
					id: 2,
					slot: 'drive',
					product_id: DRIVE_PRODUCT_ID,
					condition: 0.8,
				}),
				createSystemComponent({
					id: 3,
					slot: 'sensor',
					product_id: SENSOR_PRODUCT_ID,
					condition: 0.6,
				}),
				createSystemComponent({
					id: 4,
...
describe('getShipWithLoadout', () => {
	...
	it('throws when ship is not loaded', () => {
		const state = createState({
			systems: {
				systems: [createSystemComponent({ slot: 'core' })],
			},
		});
...
describe('getSystemStats', () => {
	...
	it('defaults nullable product fields to zero', () => {
		mockRegistry(getSystemStats, {
			[CORE_PRODUCT_ID]: createProductEmbed({
				id: CORE_PRODUCT_ID,
				rate: null,
				mult_a: null,
			}),
			[DRIVE_PRODUCT_ID]: createProductEmbed({
				id: DRIVE_PRODUCT_ID,
				sustain: null,
				mult_a: null,
				mult_b: null,
			}),
			[SENSOR_PRODUCT_ID]: createProductEmbed({
				id: SENSOR_PRODUCT_ID,
				sustain: null,
				mult_a: null,
				chance: null,

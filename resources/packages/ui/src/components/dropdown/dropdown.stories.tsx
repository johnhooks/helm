...
export const Default: Story = {
  args: {
    label: "Menu",
    renderTrigger: (props) => (
      <button {...props} type="button" style={{ color: "#f0e6d2", background: "#333", border: "1px solid #555", padding: "6px 12px", cursor: "pointer" }}>
        Open dropdown
      </button>
    ),
    children: (
        ...
        <Dropdown
          ...
          renderTrigger={(props) => (
            <button
              {...props}
              type="button"
              style={{ color: "#9f9a90", background: "none", border: "none", padding: 4, cursor: "pointer", fontSize: 18 }}
          ...
          <Panel tone="neutral" padding="sm">
            <div style={{ minWidth: 200, display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                <span style={{ fontSize: 12, color: "#f0e6d2", whiteSpace: "nowrap" }}>Star size</span>
                <SegmentedControl
                  options={[
                    { value: "sm", label: "S" },
                    { value: "md", label: "M" },
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "flex-start" }}>
        <Dropdown
          label="Demo"
          onOpenChange={(isOpen) =>
            setLog((prev) => [...prev, `${isOpen ? "opened" : "closed"} at ${new Date().toLocaleTimeString()}`])
          }
          renderTrigger={(props) => (
            <button {...props} type="button" style={{ color: "#f0e6d2", background: "#333", border: "1px solid #555", padding: "6px 12px", cursor: "pointer" }}>
              Toggle
        ...
        <div style={{ color: "#9f9a90", fontSize: 11, fontFamily: "monospace" }}>
          {log.map((entry, i) => (
            <div key={i}>{entry}</div>
          ))}
        </div>

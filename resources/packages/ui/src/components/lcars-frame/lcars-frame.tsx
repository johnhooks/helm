...
export function LcarsHeaderChip({
	children,
	className = '',
...
export function LcarsFrame({
	...
	const classNames = ['helm-lcars-frame', `helm-tone--${tone}`, className]
		.filter(Boolean)
		.join(' ');
		...
		<div className={classNames} style={style} data-testid={testId}>
			...
			<div className="helm-lcars-frame__header">
				...
				<div className="helm-lcars-frame__header-bar">
					{title && (
						<span className="helm-lcars-frame__title">{title}</span>
					)}
					{headerActions && (
						<div className="helm-lcars-frame__header-actions">
							{headerActions}
						</div>
					)}
				</div>

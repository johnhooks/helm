...
const COMPACT_TYPES = new Set< string >( [ "neutron", "pulsar", "whiteDwarf", "brownDwarf" ] );
...
export function SystemMap( {
	...
	return (
		<div
			className={ `helm-system-map ${ className }`.trim() }
			style={ style }
		>
			{ /* Primary star */ }
			{ isCompact ? (
				<div className="helm-system-map__glyph helm-system-map__glyph--star" style={ { gridRow: 1 } }>
					<StarGlyph
						spectralClass={ spectralClass }
						stellarType={ stellarType }
						size="sm"
				...
				<div
					...
					style={ { gridRow: 1, '--star-color': color } as CSSProperties }
				>
					<span className="helm-system-map__star-clip">
						<span
							className="helm-system-map__star-circle"
							style={ { width: arcDiam, height: arcDiam, background: color } }
							aria-hidden="true"
						/>
					</span>
				</div>
			...
			{ sorted.map( ( planet ) => {
				...
				const glyphSize = scanned && isPlanetType( planet.type )
				...
				return (
					<Fragment key={ planet.id }>
						<button
							type="button"
							className={ `helm-system-map__glyph${ ! scanned ? " helm-system-map__glyph--unscanned" : "" }` }
							style={ { gridRow: 1 } }
							onClick={ () => onPlanetSelect?.( planet ) }
						>
							<PlanetGlyph
								type={ scanned && isPlanetType( planet.type ) ? planet.type : "terrestrial" }
								ringed={ scanned ? planet.ringed : false }
								size={ glyphSize }
							/>
						</button>
						<span
							className={ `helm-system-map__label${ ! scanned ? " helm-system-map__label--muted" : "" }` }
							style={ { gridRow: 2 } }
						>

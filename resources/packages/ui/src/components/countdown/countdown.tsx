...
export function Countdown({
	...
	const displayTime =
		typeof remaining === 'number' ? formatTime(remaining) : remaining;

	const progress =
		total && typeof remaining === 'number'
	...
	return (
		<div
			...
			aria-label={
				label
					? `${label}: ${displayTime} remaining`
					: `${displayTime} remaining`
			}
		>
			{indicator && (
				<div className="helm-countdown__indicator">{indicator}</div>
			)}
			<div className="helm-countdown__content">
				{label && (
					<span className="helm-countdown__label">{label}</span>
				)}
				<span className="helm-countdown__time">{displayTime}</span>
				{progress !== undefined && (
					<span className="helm-countdown__progress">
						{progress}%
					</span>
				)}
			</div>

...
export function TitleBar({
	...
	const classNames = ['helm-title-bar', `helm-tone--${tone}`, className]
		.filter(Boolean)
		.join(' ');
		...
		<header className={classNames} style={style} data-testid={testId}>
			<div className="helm-title-bar__content">
				<h2 className="helm-title-bar__title">
					...
					{subtitle && (
						<>
							<span className="helm-title-bar__separator">
								{' '}
								—{' '}
							</span>
							<span className="helm-title-bar__subtitle">
								{subtitle}
							</span>
						</>
					)}
			...
			{children && (
				<div className="helm-title-bar__actions">{children}</div>
			)}
		</header>
	);
}

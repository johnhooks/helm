...
function ContextMenuActionList({ children }: ContextMenuActionListProps) {
	return <div className="helm-context-menu__actions">{children}</div>;
}
	...
	const getEnabledButtons = useCallback(
		() =>
			Array.from(actionRefs.current.values())
				.filter(
					(registration) => !registration.disabled && registration.ref
				)
...
export function ContextMenuActionItem({
	...
	return (
		<button
			...
			onClick={onClick}
		>
			<span className="helm-context-menu__action-label">{label}</span>
			{detail && (
...
export function ContextMenu({
	...
	const classNames = ['helm-context-menu', className]
		.filter(Boolean)
		.join(' ');
	...
	return (
		...
		>
			<div className={`helm-context-menu__header helm-tone--${tone}`}>
				...
				{subtitle && (
					<span className="helm-context-menu__subtitle">
						{subtitle}
					</span>
				)}
			</div>

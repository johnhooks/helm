...
const ShipCard = ({
	...
	mode: PowerModeKey;
	onModeChange: (m: PowerModeKey) => void;
}) => {
			...
			>
				...
				>
					...
					<div style={readoutGrid}>
						...
						<EngCell
							label="Discovery"
							value={BASE.discovery}
							unit="%"
							level={BASE.discovery}
							tone="lilac"
						/>
					</div>
	...
	>
		...
		<div
			style={{
				...
				animation: !star.name
					? `helm-twinkle ${3 + (star.x % 4)}s ease-in-out ${
							star.y % 3
					  }s infinite`
					: undefined,
			}}
						...
						{selected && (
							<div
								style={{
									...
									border: `1.5px solid ${C.sky}`,
									animation:
										'helm-pulse-ring 2s ease-in-out infinite',
...
const completedEntries = (
	<>
		...
		<LogCard
			time="Y1 D11"
			title="Scan complete — Tau Ceti IV"
			tone="lilac"
			status={
				<StatusBadge tone="success" size="sm">
					Complete
...
const LogShell = ({ children }: { children: ReactNode }) => (
	...
	>
		<div style={{ ...labelStyle, padding: '2px 0', flexShrink: 0 }}>
			Ship Log
		</div>
...
const IdleLog = () => <LogShell>{completedEntries}</LogShell>;
...
export const Default: Story = {
	...
	render: () => <BridgeLayout view={<StarfieldView />} log={<IdleLog />} />,
};
...
export const Active: Story = {
	args: { children: null },
	parameters: { controls: { disable: true } },
	render: () => <BridgeLayout view={<StarfieldView />} log={<ActiveLog />} />,
};
...
export const Survey: Story = {
	args: { children: null },
	parameters: { controls: { disable: true } },
	render: () => <BridgeLayout view={<SurveyView />} log={<IdleLog />} />,
};

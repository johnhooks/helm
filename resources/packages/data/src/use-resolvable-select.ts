...
type ResolvableSelectorsOf< S > = S extends StoreDescriptor< infer C >
	? C extends { selectors?: infer Sel }
		? {
			[ K in keyof Sel ]: Sel[ K ] extends ( state: any, ...args: infer A ) => infer R
				? ( ...args: A ) => Resolvable< R >
				: Sel[ K ];
		}
		: never
...
function enrichSelectors< S extends Record< string, ( ...args: unknown[] ) => unknown > >(
	...
	return enriched as Record< string, ( ...args: unknown[] ) => Resolvable< unknown > >;
}
	...
	return useSelect(
		( select ) => {
			const resolve = ( store: StoreDescriptor ) =>
				enrichSelectors( select( store ) as WithMeta< Record< string, ( ...a: unknown[] ) => unknown > > );
			// eslint-disable-next-line @typescript-eslint/no-explicit-any

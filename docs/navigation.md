...
Both improve with upgrades, but they gate different things. A ship with a great nav computer but a weak core can _discover_ long routes but has to hop slowly along them. A ship with a great core but a basic nav computer can jump far but can only find paths to nearby stars.

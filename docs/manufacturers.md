...
- DSC Mk I: 7 ly sustain range, 2.0x survey time, 0.6 accuracy
...
- VRS Mk I: 5 ly sustain range, 1.0x survey time, 0.7 accuracy
...
- ACU Mk I: 3 ly sustain range, 0.5x survey time, 0.85 accuracy
...
- Aegis Delta: Balanced coverage. The "you probably want this one" shield.
- Aegis Eta: Heavy capacitor banks, massive absorption, slow to recharge. For pilots who expect to take one big hit.
...
### Null Point Labs (Stealth Systems)

*"We make nothing — precisely."*

Null Point Labs started in military electronic warfare — building systems that made ships invisible to enemy sensors during the Internet Wars. While Aegis was armoring hulls to survive detection, Null Point was ensuring detection never happened in the first place. When the civilian market opened, their technology found an unexpected home: miners, smugglers, and scouts who valued not being found.

Their core product is the Veil Array — an EM emission suppression field that dampens a ship's electromagnetic signature across all spectral bands. It doesn't make you invisible; it makes you quieter. The math is simple: if your emission drops below the noise floor, you don't exist on anyone's sensor readout.

What makes Null Point dangerous isn't just what they suppress — it's what they understand. Years of EM warfare research means they know exactly how sensors work, what signatures betray a ship, and which emission patterns are hardest to filter from noise. This knowledge cuts both ways. Their engineers know how to hide, and they know how to find what's hiding.

**Product line:**
- Veil Array: EM emission suppression field. High base power draw (0.40), practically requires the Specter hull's stealth draw bonus to operate alongside weapons.

**Crossover product:**
- NPL Sensor (passive hunter): A sensor array designed to detect ships, not stars. Extreme passive affinity, optimized for identifying drive emissions, weapon fire, and shield harmonics. Poor active scanning. Useless for exploration — built for the hunt. *See Crossover Products below.*

**Design language:** Minimalist. Flat black surfaces, no running lights, no exposed components. Null Point products look like they're trying not to be noticed, because they are.

**Personality:** Quiet, precise, slightly unsettling. Null Point engineers don't brag. They present data sheets with redacted military heritage citations and let the specs speak. The company culture is "the best outcome is one where nobody knows you were there."
...
Null Point Sensor (NPL)
├── Range: 3 ly (worst in class)
├── Survey speed: 2.5x (terrible — not built for surveying)
├── Accuracy: 0.4 (worst in class for exploration)
├── Passive affinity: 1.8 (best in game — far beyond DSC)
├── Active affinity: 0.3 (almost useless — Null Point doesn't broadcast)
├── BUT: detects ship emissions at extreme sensitivity
├── BUT: identifies drive class, weapon type, and shield state from passive data alone
└── "They don't look for planets. They look for pilots."
```
...
- **Balance as manufacturing** — when a product needs a buff or nerf, the manufacturer releases a new version (a retooled production run). The old version stops manufacturing but every unit already in the game stays exactly as it was — it can be used, repaired, bought, sold, and salvaged, but not remanufactured.

The database tracks this with the `version` column (manufacturing run) and a `released_at` timestamp. A product that hasn't been released yet doesn't appear in station inventories or manufacturing recipes. When a new version releases, only the current version is available for manufacture — previous runs become legacy hardware that persists in the economy through player ownership and trade.

This means balance changes create economic ripples. A nerfed-in-v2 product where v1 was strong? Those v1 units become collectibles — scarce, sought-after, priced accordingly. A buffed-in-v2 product where v1 was weak? The old stock becomes cheap beaters for new players. The game's economy remembers every manufacturing run.

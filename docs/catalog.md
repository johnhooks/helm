...
The hardware that keeps you alive between the stars.

Every component listed here is manufactured by a company with opinions. Those opinions show up in the engineering. A DR-Series drive doesn't feel like an Aegis drive because DR-Series has been building drives for thirty years and Aegis thinks everything is a shield problem. Choose accordingly.
...
The tradeoff is real: every system on your ship that depends on core output — your drive's performance ratio, your sensor's comfort range — is running at 90% of what an S-rated core would give. That 10% matters over long jumps. But the Endurance will still be running when the Rapid is dead.
...
This isn't a criticism. The Standard exists at 1.0 because Epoch deliberately designed it as the reference point. 750 HP, 10.0 regen rate, honest numbers across the board. It's the core that ships with the Pioneer frame because it doesn't lie about anything. You always know exactly where you stand.
...
This is the core for short-haul pilots, combat escorts, and anyone whose mission profile is "get in, do the thing, get back before the core degrades." Paired with a DR-705, a Rapid will get you places faster than anything in the catalog. Paired with a DR-305, it's almost wasteful — all that regen and nowhere to spend it.
...
Founded by a collective of drive engineers who left a megacorp to build drives without committee-designed compromises. Every DR drive is named by its power class — the number tells you everything. Higher number, more thrust, more stress.
...
But here's the thing the 305 pilots know: within that 10-light-year comfort range, there's no strain penalty. The 305 jumps 10 ly for the same proportional cost that the 505 jumps 7. And the 305 does it on a third of the power. For trade routes, supply runs, and any mission where time is cheaper than fuel, the 305 is the rational choice.
...
The spool curve is linear (1.0) — 4 minutes of steady buildup to 1.5 peak power. No front-loaded spike, no gentle warmup. The 505 spools the way it does everything: predictably. Cooldown takes 3 minutes and runs a little hot (curve 1.2). It's the longest cooldown signature of any standard drive, which means a DSC array has more time to hear you arrive.
...
But the 705 isn't about efficiency. It's about being somewhere else, right now. Combat escorts run 705s because reaction time matters more than fuel budgets. Rescue operations run 705s because the distance-to-survivor equation only goes one direction. Smugglers run 705s because nobody outruns a 305.

The spool signature is unmistakable — front-loaded curve (0.5) reaching 2.5 peak power in 2 minutes. That's a hard electromagnetic spike that lights up every sensor array in the neighborhood. An ACU will hear you spool from the next system over. A DSC will know what drive class you're running before you finish warming up. The 705 trades stealth for speed and doesn't apologize.
...
The numbers are mediocre on purpose. Sustain of 6.0 ly — decent but unremarkable. Amplitude of 0.7x — noticeably slow. Consumption at 0.8x — efficient, but not 305-efficient. Power draw of 3.0. On paper, the SDR is an inferior drive in every measurable category. That's because the only category that matters isn't on the paper.

The spool signature peaks at 0.5 power — less than half a DR-305, a fifth of a DR-705. The curve is aggressively back-loaded (2.0), meaning the SDR spends 5 full minutes reaching peak output so gradually that the emission ramp disappears into the noise floor. Sustain power sits at 0.3. Cooldown peak is 0.4 over 3 minutes. The SDR in transit is quieter than most ships at idle.

A DSC array trained on an SDR's transit corridor might — might — pick up the signature after hours of integration. An ACU array, tuned for hard electromagnetic pulses, will hear essentially nothing. The SDR doesn't spool like a drive. It seeps.

The throttle caps at 1.5, which means even in overdrive the SDR never produces the kind of emission spike that triggers detection. You can push it and still remain below the noise floor in most environments.
...
The sensor market is the most competitive vertical in the hardware industry. Three companies — Deep Scan Dynamics, Versa Instruments, and Acuity Systems — have spent decades building products that embody fundamentally different philosophies about what a sensor should prioritize. They genuinely dislike each other's marketing. This is good for pilots.
...
The DSC philosophy is passive-first. Their arrays ship with the highest passive affinity in the industry (1.40) and the strongest continuous-emission gain (1.50). A DSC array sitting quietly in an asteroid belt, listening, will detect a miner's continuous emission signature hours before a VRS array in the same position. It will detect it days before an ACU even registers the signal.

Active scanning is not the DSC's strength. With 0.70 active affinity and 0.70 pulse gain, active sweeps are noticeably weaker per-sweep than the competition. DSC compensates with sheer integration time — their sensor array keeps accumulating passive data, building confidence over hours. The DSC pilot's mantra is "let the antenna do the work."
...
**Mk II:** Sustain extends to 8.0 ly, accuracy improves to 68%, survey time drops to 1.4x. Passive reaches 1.50, continuous gain hits 1.60. The active affinity climbs too — 0.80 at v1, capped at 0.85 by v3 — suggesting DSC is reluctantly conceding that sometimes you need to ping. But there's a hard ceiling: DSP identity floors ensure the DSC never closes the gap on VRS for active work. They're still a passive-first array, and the engineering enforces it.
...
Every DSP parameter on a VRS array is within spitting distance of 1.0. Active: 1.0. Passive: 1.0. Pulse gain: 1.0. Continuous gain: 1.0. The VRS is the reference sensor the way the Epoch-S is the reference core. It exists at the center of every axis.

This makes the VRS boring on paper and excellent in practice. A VRS pilot never looks at a detection scenario and thinks "my sensor can't do this." It can always do it. Just not as well as the specialist. The VRS pilot's advantage is that they never need to know in advance what they're looking for.
...
**Mk II:** Sustain climbs to 5.8 ly, accuracy to 78%, survey time drops to 0.85x. Every DSP parameter nudges to 1.08. The VRS Mk II is a better generalist — slightly above average at everything instead of exactly average. Versa's pitch is honest: "we made it better at all of it."
...
The ACU is an active-first sensor. Active affinity of 1.40, pulse gain of 1.50. When you fire an active scan with an ACU, each sweep hits harder than any other array in the catalog. Passive affinity sits at 0.60 — the ACU barely listens. It interrogates. The continuous gain is 0.70, meaning the patient listening game that DSC excels at is nearly useless here.

This makes the ACU the predator's sensor. You know something is there. You want to know what it is, right now, and you don't care that your active ping just told them you're looking. Combat escorts, customs patrols, bounty hunters — anyone whose job involves finding a specific thing in a specific place runs ACU.
...
**Mk II:** Sustain extends to 4.0 ly, accuracy reaches 90%, survey time drops to 0.42x. Active hits 1.50, pulse gain 1.60. The passive affinity improves — 0.70 at v1, capped at 0.75 by v3 — suggesting Acuity finally conceded that pilots sometimes need their sensor to work when they aren't actively pinging. But DSP identity floors keep the ACU's passive ceiling well below neutral. The continuous gain tops out at 0.85. The ACU Mk II is still, fundamentally, a spotlight, not a stethoscope.
...
What the MIL array offers is something no civilian sensor provides: genuinely high capability at both active scanning and passive awareness, at scale. Active affinity of 1.50 with a passive affinity of 0.80 at v1 (capped at 0.85 by v3) — the only sensor in the catalog where both numbers are above baseline. Pulse gain of 1.40, continuous gain of 0.85 at v1 (capped at 0.90). The MIL isn't as good at active scanning as a dedicated ACU, and it isn't as good at passive listening as a DSC. But it's uncomfortably close to both — and DSP identity floors ensure "uncomfortably close" never becomes "indistinguishable."

The cost is printed in every other number. Footprint of 30 (twice an ACU). Power draw of 4.0 (the highest in the catalog). Survey time of 2.5x (the slowest). The MIL array was designed for ships with power budgets and internal space that civilian frames don't have. Installing one on a Pioneer means sacrificing cargo capacity and power margin that most loadouts can't spare.

Sustain of 4.0 ly and a base detection chance of 75% — the MIL doesn't see as far as a DSC or as accurately as an ACU within range. It sees well enough at both, which is what a military sensor needs. It needs to find threats (active, pulse-dominant) and maintain awareness (passive, continuous). It needs to do both without choosing.
...
The answer is a sensor that reads core harmonics instead of the EM spectrum. It's terrible at almost everything a sensor is supposed to do. Footprint of 20. Range of 3.5 ly at v1 (reaching 4.0 at v3 — barely matching an ACU). Detection chance of 45% (the worst in the catalog by a wide margin). Survey time of 1.80x (slower than a DSC). The Epoch Sensor is objectively the worst sensor in the catalog by every standard metric.
...
The Alpha is for pilots who expect combat to be a series of exchanges, not a single catastrophic hit. Skirmishers. Scouts. Anyone whose plan is "don't be where the big hit lands."
...
Like the Epoch-S and the DR-505, the Delta exists at the center. It's not the best at any single metric. It's acceptable at all of them. A Pioneer-class frame with an Aegis Delta will survive the threats that the designers expected. It won't survive the threats that require an Eta.
...
Footprint of 35. Power draw of 8.0. This is a heavy shield for heavy ships. Installing an Eta on a Pioneer means sacrificing cargo and power that most loadouts need. It's a statement: this ship expects trouble, and it plans to stand in the middle of it.

Priority range of 0.5x to 3.0x — the highest ceiling in the catalog. At 3.0 priority, the Eta regens at 15.0 and pulls 24.0 power. That's nearly the entire output of an Epoch-S devoted to shield recovery. Pilots who run Eta at max priority tend to pair it with an Epoch-R, because the power budget demands it.
...
But while the shield field is active — any charge remaining, even 1 unit — the shield's wavefront geometry acts as a passive sensor amplifier, extending passive detection range by 40%. For a DSC Mk II pilot who already sees further than anyone, that 40% bonus is transformative. It turns "I can probably detect that" into "I've been watching that for hours."

The power draw is just 2.0 — lower than any Aegis shield — because the DSC Shield is barely generating a defensive field. It's generating a sensor field shaped like a shield. The 15 footprint is bigger than the Alpha's 10, which is where the antenna geometry lives.
...
The nav computer determines discovery probability — the chance of finding something interesting at each hop of an exploration route. Higher tiers have better skill multipliers and efficiency curves, which translates directly to more discoveries per expedition.

### Tier 1

Skill: 0.3x. Efficiency: 0.5x. The nav stack that ships with every frame. It works. It finds the obvious routes and the obvious discoveries. Pilots upgrading from Tier 1 universally describe the experience as "I didn't know what I was missing."

### Tier 2

Skill: 0.45x. Efficiency: 0.6x. The first upgrade most pilots buy. The discovery probability improvement is immediately noticeable — routes that returned nothing on Tier 1 start showing results. Available at any station with a software vendor.

### Tier 3

Skill: 0.6x. Efficiency: 0.7x. The standard recommendation for exploration-focused pilots. Tier 3 represents the point where the nav computer starts reliably finding non-obvious discoveries. This is the tier that ships with the Pioneer's recommended loadout. Most pilots stay here.

### Tier 4

Skill: 0.75x. Efficiency: 0.85x. Specialist exploration software. The jump from Tier 3 to Tier 4 is where the nav computer starts to matter for deep-range survey missions. Discovery probability at depth holds up better against the exponential decay — the efficiency curve means each additional hop costs less discovery potential.

### Tier 5

Skill: 0.9x. Efficiency: 1.0x. The best available. Tier 5 at efficiency 1.0 means zero waste in the discovery calculation — every bit of the pilot's skill and the nav computer's algorithms contribute to the probability. The difference between Tier 4 and Tier 5 is subtle at shallow depth and significant at deep exploration. Pilots running Tier 5 find things at hop 5 that Tier 3 pilots wouldn't find at hop 2.

Available at major stations only. Pricing reflects scarcity, not manufacturing cost — it's the same hardware running better code.

---

## Reading the Numbers

For pilots who want to understand the spec sheets.
